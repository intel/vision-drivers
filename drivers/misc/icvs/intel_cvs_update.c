...
int cvs_write_i2c(u16 cmd, u8 *data, u32 len)
{
	...
	case SET_HOST_IDENTIFIER:
		u8 *out_buff;
		out_buff = devm_kzalloc(ctx->dev,(sizeof(u32) + sizeof(cmd)), GFP_KERNEL);
		if (!out_buff) {
		...
		memcpy(&out_buff[2], &ctx->host_identifiers.value, sizeof(u32));

		count = i2c_master_send(client, (const char *) out_buff,
					sizeof(cmd)+sizeof(u32));

		mdelay(GPIO_WRITE_DELAY_MS);
		if (count != sizeof(u32) +sizeof(cmd))
			return -EIO;
...
int cvs_dev_fw_dl_start(void)
{
	...
	status = cvs_get_device_state(&fw_state);
	if ((status) ||
	    ((fw_state & DEVICE_DWNLD_STATE_MASK) != DEVICE_DWNLD_STATE_MASK)) {
		status = -EIO;
		dev_err(cvs->dev,
